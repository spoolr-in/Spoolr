...
namespace SpoolrStation.Services
{
    public class DocumentService : IDocumentService
    {
        ...
        private readonly ConcurrentDictionary<long, string> _tempFilePaths = new();
        ...
        public async Task<DocumentStreamResult> GetDocumentPreviewAsync(DocumentPrintJob job, string? authToken)
        {
            try
            {
                ...
                var streamResult = await StreamDocumentToMemoryAsync(job.StreamingUrl, job.JobId, authToken);
                if (!streamResult.Success)
                {
                    return streamResult;
                }

                var tempFilePath = await CreateTempFile(streamResult.Data, job.OriginalFileName);
                _tempFilePaths[job.JobId] = tempFilePath;

                return DocumentStreamResult.CreateSuccess(streamResult.Data, streamResult.ContentType, tempFilePath);
            }
            catch (Exception ex)
            {
        ...
        public async Task CleanUpJobDataAsync(long jobId)
        {
            await Task.Run(() =>
            {
                ClearDocumentCache(jobId);

                if (_tempFilePaths.TryRemove(jobId, out var tempFilePath))
                {
                    try
                    {
                        if (File.Exists(tempFilePath))
                        {
                            ...
                            _logger.LogInformation($"Deleted temporary file for job {jobId} at {tempFilePath}");
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, $"Failed to delete temporary file for job {jobId} at {tempFilePath}");
                    }
                }
            });
        }

        private async Task<string> CreateTempFile(byte[] data, string originalFileName)
        {
            var tempPath = Path.Combine(Path.GetTempPath(), $"spoolr_preview_{Guid.NewGuid()}_{originalFileName}");
    ...
    public class DocumentStreamResult
    {
        ...
        public static DocumentStreamResult CreateSuccess(byte[] data, string contentType, string? tempFilePath = null)
        {
            return new DocumentStreamResult { Success = true, Data = data, ContentType = contentType, TempFilePath = tempFilePath };
        }
