...
public class MainViewModel : BaseViewModel
{
    ...
    public ObservableCollection<CompletedJobModel> CompletedJobs { get; private set; }
    public bool HasCompletedJobs => CompletedJobs.Any();
    ...
    public MainViewModel(AuthService? authService = null, ILoggerFactory? loggerFactory = null)
    {
        ...
        CompletedJobs = new ObservableCollection<CompletedJobModel>();
        ...
        CompleteJobCommand = new RelayCommand<AcceptedJobModel>(async (job) => await MarkJobAsCompletedAsync(job), CanExecuteJobAction);
    ...
    private async Task MarkJobAsCompletedAsync(AcceptedJobModel? job)
    {
        if (job == null) return;

        StatusText = $"Marking job {job.JobId} as completed...";

        var success = await (_webSocketClient?.UpdateJobStatusToCompletedAsync(job.JobId) ?? Task.FromResult(false));

        if (success)
        {
            var completedJob = new CompletedJobModel
            {
                JobId = job.JobId,
                CustomerName = job.Customer,
                FileName = job.FileName,
                CompletedAt = DateTime.Now,
                FinalPrice = job.TotalPrice,
            ...
            await documentService.CleanUpJobDataAsync(job.JobId);

            StatusText = $"Job {job.JobId} completed and moved to history.";
        }
        else
        {
            StatusText = $"Failed to mark job {job.JobId} as completed.";
        }
    }
