    ...
    public class DocumentPreviewViewModel : BaseViewModel, IDisposable
    {
        ...
        private async Task LoadDocumentAsync()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    var fileType = _documentService.DetectFileType(result.ContentType, Job.OriginalFileName);
                    IsPdfDocument = fileType == SupportedFileType.PDF;

                    string tempFilePath = Path.Combine(Path.GetTempPath(), $"spoolr_preview_{Guid.NewGuid()}");

                    switch (fileType)
                    {
                        case SupportedFileType.DOCX:
                            var htmlResult = await _documentService.ConvertDocxToHtmlAsync(result.Data, Job.JobId);
                            if (htmlResult.Success) { tempFilePath = Path.ChangeExtension(tempFilePath, ".html"); await File.WriteAllBytesAsync(tempFilePath, htmlResult.Data); } else { throw new Exception(htmlResult.ErrorMessage); }
                            break;
                        case SupportedFileType.JPG:
                        case SupportedFileType.PNG:
                            var imageHtmlResult = await _documentService.PrepareImageForDisplayAsync(result.Data, result.ContentType, Job.JobId, Job.OriginalFileName);
                            if (imageHtmlResult.Success) { tempFilePath = Path.ChangeExtension(tempFilePath, ".html"); await File.WriteAllBytesAsync(tempFilePath, imageHtmlResult.Data); } else { throw new Exception(imageHtmlResult.ErrorMessage); }
                            break;
                        default:
                            tempFilePath = Path.ChangeExtension(tempFilePath, ".pdf");
                            await File.WriteAllBytesAsync(tempFilePath, result.Data);
                            break;
                    }
                    DocumentSource = tempFilePath;
                }
                else
                {
        ...
        private async Task ExecutePrint()
        {
            try
            {
                ...
                if (!printService.ValidatePrinterCapabilities(SelectedPrinter.PrinterName, Job.PrintSpecs, out string validationMsg))
                {
                    var result = System.Windows.MessageBox.Show(
                        $"Warning: {validationMsg}\n\nDo you want to continue anyway?",
                        "Printer Compatibility Warning",
